  ...
  const screens = [
    ...
    <div key="home" style={{ height: "100%", padding: "28px 16px 16px", display: "flex", flexDirection: "column", animation: "screenFade 0.45s ease both" }}>
      ...
      {[["Pain Relief","#4f8ef7"],["Antibiotics","#9f5cf7"]].map(([l,c]) => (
        <div key={l} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 9, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", marginBottom: 7 }}>
    ...
    <div key="map" style={{ height: "100%", padding: "28px 0 0", animation: "screenFade 0.45s ease both" }}>
      ...
      <div style={{ padding: "0 12px" }}>
        {[["Raydos Pharmacy","0.3 km","#5ff7c4"],["Klan Pharmacy","0.8 km","#4f8ef7"]].map(([n,d,c]) => (
          <div key={n} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 10px", borderRadius: 8, background: "rgba(255,255,255,0.04)", border: `1px solid ${c}22`, marginBottom: 6 }}>
            <span style={{ fontSize: 9, color: "rgba(255,255,255,0.65)", fontWeight: 500 }}>{n}</span>
            <span style={{ fontSize: 8, color: c, fontWeight: 700 }}>{d}</span>
          </div>
        ))}
      </div>
    </div>,
    ...
    <div key="detail" style={{ height: "100%", padding: "28px 14px 14px", display: "flex", flexDirection: "column", animation: "screenFade 0.45s ease both" }}>
      ...
      {[["Paracetamol 500mg","₵2.50","In Stock","#5ff7c4"],["Ibuprofen 400mg","₵3.80","Low Stock","#f7c14f"]].map(([n,p,s,c]) => (
        <div key={n} style={{ padding: "8px 10px", borderRadius: 9, background: "rgba(255,255,255,0.04)", border: `1px solid rgba(255,255,255,0.07)`, marginBottom: 7 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 9, color: "rgba(255,255,255,0.65)", fontWeight: 500 }}>{n}</span>
            <span style={{ fontSize: 10, color: "#fff", fontWeight: 800 }}>{p}</span>
          </div>
          <div style={{ marginTop: 3, fontSize: 8, color: c, fontWeight: 700 }}>{s}</div>
        </div>
      ))}
            ...
            {[["Locate pharmacies instantly","#4f8ef7"],["Real-time stock updates","#9f5cf7"],["24/7 delivery tracking","#00d4ff"]].map(([label, color]) => (
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: `${color}18`, border: `1px solid ${color}40`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round"><path d="M20 6L9 17l-5-5" /></svg>
                </div>
                <span style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", fontWeight: 400 }}>{label}</span>
              </div>
            ))}
